...
using Spots_Barrier = LaserTag.Core.Model.Spots.Barrier;


namespace LaserTag.Core.Model.Body;
...
public class PlayerBodyLayer : RasterLayer, ISteppedActiveLayer
{
    ...
    {
        _scoringStrategy.CalculateScore();
    ...
    public Spots_Barrier NearestBarrier(Position position) =>
        (Spots_Barrier) SpotEnv.Explore(position, -1, 1, spot => spot.GetType() == typeof(Spots_Barrier)).FirstOrDefault();
    ...
    private Spot CreateSpots(double type, Position position)
    {
        ...
        {
            case 1:
                spot = AgentManager.Spawn<Spots_Barrier, PlayerBodyLayer>(null, s => s.Position = position).Take(1).First();
                break;
            case 2:
    ...
    /// <summary>
    ///   Sets the scoring strategy based on the game mode.
    /// </summary>
    /// <exception cref="NotSupportedException"></exception>
    private void SetScoringStrategy()
    {
        _scoringStrategy = Mode switch
        {
            GameMode.TeamDeathmatch => new TeamDeathmatchScoring(this),
            GameMode.CaptureTheFlag => new CaptureTheFlagScoring(this),
